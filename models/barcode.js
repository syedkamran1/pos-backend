...
const Barcode = {
  createBarcodePdf: async (barcode, quantity) => {
    try {
      logger.info(
        `Starting PDF creation for barcode: ${barcode}, quantity: ${quantity}`
      );
      ...
      try {
        ...
        if (!fs.existsSync(folderPath)) {
          ...
          logger.info(
            `Folder barcode created successfully in the current directory.`
          );
        } else {
          logger.info(
            `Folder barcode already exists in the current directory.`
          );
        }
      } catch (error) {
      ...
      logger.error(`Error creating barcode PDF: ${error.message}`, {
        barcode,
        quantity,
      });
  ...
  printPdf: async (pdfFileName) => {
    try {
      ...
      logger.error(`Failed to print ${pdfFileName}: ${error.message}`, {
        stack: error.stack,
      });
    }
  },
